...
## Cloudflare Pagesで更新後に必ず行う確認

Service Workerのキャッシュを更新した後は、iPad側に古いキャッシュや古いホーム画面アプリが残ることがあります。
特に `Response served by worker has redirections` が出た場合は、以下を最初から行ってください。

1. Cloudflare Pagesへ再デプロイします。
2. iPadの古いホーム画面アイコンを削除します。
3. iPadのSafariで、履歴とWebサイトデータから公開URLのサイトデータを削除します。
4. iPadのSafariでHTTPS URLを開きます。
5. 30秒待ちます。
6. Safariの共有ボタンから「ホーム画面に追加」を選びます。
7. ホーム画面アイコンからオンラインで一度起動します。
8. `START` を押してから `RESET` を押します。
9. iPadを機内モードにします。
10. アプリを完全終了します。
11. ホーム画面アイコンから再起動します。
12. もう一度アプリを完全終了します。
13. もう一度ホーム画面アイコンから再起動します。

この2回目の完全終了後の再起動まで通れば、ネットのない体育館で使える状態です。
